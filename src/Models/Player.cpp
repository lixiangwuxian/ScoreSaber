...
Player::Player(rapidjson::Value const& userModInterface) {
    ...
    id = userModInterface["id"].IsNull() ? "" : userModInterface["id"].GetString();
    name = userModInterface["name"].IsNull() ? "" : userModInterface["name"].GetString();
    country = userModInterface["country"].IsNull() ? "" : userModInterface["country"].GetString();
    avatar = userModInterface["profilePicture"].IsNull() ? "" : userModInterface["profilePicture"].GetString();
    role = userModInterface["role"].IsNull() ? "" : userModInterface["role"].GetString();
    pp = userModInterface.HasMember("pp") ? userModInterface["pp"].GetFloat() : 0.0f;
    rank = userModInterface.HasMember("rank") ? userModInterface["rank"].GetInt() : 0;
    countryRank = userModInterface.HasMember("countryRank") ? userModInterface["countryRank"].GetInt() : 0;
...
void Player::SetHistory(rapidjson::Value const& history) { //todo
